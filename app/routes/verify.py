...
from app.models import MobileOTP, Mobile, Application
...
def application_status():
    ...
    # Step 1: Find the mobile entry
    mobile_entry = Mobile.query.filter_by(mobile=mobile).first()
    if not mobile_entry:
        return jsonify({'status': 404, 'error': 'Mobile number not found'}), 404

    # Step 2: Check if an application exists for this mobile
    app_record = Application.query.filter_by(mobile_id=mobile_entry.id).first()
    if not app_record:
        return jsonify({'status': 404, 'error': 'No application found for this mobile', }), 404

    # Step 3: Return additional info
    return jsonify({
        'status': 200,
        'data': {
            'mobile': mobile,
            ...
            # static for now; make dynamic if you add a 'status' column later
